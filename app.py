...
def admin():
    ...
    if st.session_state.login==True:
        if username=="Admin" and password=='admin123':
            ...
            # db.commit()
            tables =cursor.fetchall()
            showtableform = pd.DataFrame(tables, columns=[
                          'studentname','id','re_date','status'])
            st.write(showtableform)
            for i in tables:
                st.write(i[1])
                st.write(i[2])
                Accept=st.button('Accept',key=i[0])
                if Accept:
                    st.write('Congratulation !! You are Registered for the Camp')
                    cursor.execute(f"Update information set status='Accepted' where id='{i[0]}'")
                    db.commit()
                Reject=st.button('Reject',key=i[0])
                if Reject:
                    st.write('Sorry you are not selected')
